...
- **Infra**: Terraform com ossos do ofício (VPC, RDS, ECS, S3, CloudWatch) pronto para quando você sair do modo free.
...
- **Backend**: pode ser publicado no Render/Railway/fly.io. Basta apontar `npm run start` e definir as envs desejadas.
...
- Artefatos de compliance estão em `docs/`: `lgpd_checklist.md`, `security_checklist.md` e `dpa_template.md` para contratos com parceiros.

## Observabilidade

- Configure `SENTRY_DSN`, `SENTRY_ENVIRONMENT` e `SENTRY_TRACES_SAMPLE_RATE` para capturar erros e traces. Detalhes em `docs/monitoring.md`.
- No AWS deploy, CloudWatch já recebe métricas/logs; use os exemplos de dashboard/alarme descritos no mesmo documento.
